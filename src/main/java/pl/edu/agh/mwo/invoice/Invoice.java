...
public class Invoice {
    ...
    public BigDecimal getSubtotal() {
        ...
        for (Map.Entry<Product, Integer> position : quantityOfProducts.entrySet()) {
            getSubTotal = getSubTotal.add(position.getKey().getPrice()
                    .multiply(BigDecimal.valueOf(position.getValue())));
        }
    ...
    public BigDecimal getTax() {
        ...
        for (Map.Entry<Product, Integer> position : quantityOfProducts.entrySet()) {
            tax = tax.add(position.getKey().getTaxPercent().multiply(position.getKey()
                    .getPrice()).multiply(BigDecimal.valueOf(position.getValue())));
        }
    ...
    public BigDecimal getTotal() {
        ...
        for (Map.Entry<Product, Integer> position : quantityOfProducts.entrySet()) {
            totalPrice = totalPrice.add(position.getKey().getPriceWithTax().multiply(BigDecimal
                    .valueOf(position.getValue())));
        }
        return totalPrice;
    }
    ...
    public String printListOfProducts() {
        ...
        for (Map.Entry<Product, Integer> position : quantityOfProducts.entrySet()) {
            BigDecimal price = position.getKey().getPriceWithTax().multiply(BigDecimal
                    .valueOf(position.getValue()));
            message.append(position.getKey().getName() + " "
                    + position.getValue() + " " + price + "\n");
        }
